...
Script: mc_client.py
By:Richard Vaughan
Purpose:Multicast client for Multicast communication to server.
Prerequisites: None.
...
while True:
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_UDP) as s:
        # inet_aton converts IPv4 from the dotted decimal string to 32 bit packed binary format
        s.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP,
                     socket.inet_aton(MCAST_GRP) + socket.inet_aton(MCAST_IF_IP))
